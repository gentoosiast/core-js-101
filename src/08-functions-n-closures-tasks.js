...
function getPolynom(...coefficients) {
  const coeffs = [...coefficients].reverse();
  ...
  return function getCoeff(x) {
    ...
    return coeffs.reduce((acc, val, i) => acc + val * x ** i, 0);
  };
}
...
function memoize(func) {
  let memoizedFunction = func;
  let result = null;
  return () => {
    if (memoizedFunction) {
      result = memoizedFunction();
...
function logger(func, logFunc) {
  return function logWrapper(...args) {
    const funcArgs = args.map((arg) => JSON.stringify(arg));
...
function partialUsingArguments(fn, ...args1) {
  return function partialFunc(...remainingArgs) {
